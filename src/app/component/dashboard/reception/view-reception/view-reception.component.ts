...
export class ViewReceptionComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.getAllPatientsForReception();
  }

  getReceptionById() {
    this.dataApi.getReceptionistById(this.id).subscribe(res => {
      this.receptionObj = res;
    })
  }

  getAllPatientsForReception() {
    this.dataApi.getAllPatients().subscribe(res => {
      this.allPatients = res.map((e : any) => {
        const data = e.payload.doc.data();
        if(data.reception_id == this.id) {
